...
def roll_dice(num_rolls, dice=six_sided):
    ...
    pig_out = 1
    is_pig_out = 0
    while timesrolled != num_rolls:
        die_value = dice()
        timesrolled += 1
        total_value += die_value
        if die_value == 1:
            is_pig_out = 1
    if is_pig_out == 1:
            total_value = pig_out
...
def free_bacon(score):
    ...
    ones_digit = score % 10
    tens_digit = score // 10
    digit = min(ones_digit, tens_digit)
    gain = 10 - digit
    return gain
    # END PROBLEM 2
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def announce_new_highest(score0, score1):
        gain = 0
        player = who
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while num_rolls <= 10:
        ...
        get_average = average(num_rolls, dice)
        if get_average > record:
            max_rolls = num_rolls
            record = get_average
        num_rolls += 1
    return max_rolls
